      ...
      <ul className="space-y-6">
        {commits?.map((commit,index) => {
          return <li key={commit.id} className="relative flex gap-x-4">
           <div className={cn(index === commits.length - 1 ? 'h-6' : '-bottom-8', 'absolute left-0 top-0 flex w-6 justify-center')}>
            ...
            <>
              ...
              <div className="flex-auto rounded-md bg-white p-3 ring-1 ring-inset ring-gray-200">
                <div className="flex justify-between gap-x-4">
                  <Link target="_blank" href={`${project?.repoUrl}/commit/${commit.commitHash}`} className="py-0.5 text-xs leading-5 text-gray-500">
                    <span className="font-medium text-gray-800">{commit.commitAuthor}</span>
